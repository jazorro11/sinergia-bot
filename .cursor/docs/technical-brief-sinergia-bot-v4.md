...
│   ├── prompts.py       # System prompt principal, prompt post-Calendly y extracción
...
- `prompts.py` contiene los tres prompts como constantes de texto (conversación, despedida post-Calendly, extracción). Ningún otro módulo construye prompts inline.
...
> **Fuente de verdad:** el texto del recuadro siguiente coincide con la constante `SYSTEM_PROMPT` en `bot/prompts.py` (revisión v4.1 del brief).

Este es el system prompt que se pasa al LLM en cada llamada conversacional **mientras la captura está activa**. Define personalidad, reglas de conversación, los 9 datos a recopilar y cuándo enviar la URL de Calendly.
...
### System prompt post-Calendly (despedida)

Constante `SYSTEM_PROMPT_POST_CALENDLY_FAREWELL` en `prompts.py`. Sustituye al system principal cuando el usuario escribe **después** de que el asistente ya envió el enlace de Calendly en el historial.

**Cuándo se usa (`conversation.py`):** si el lead está en estado `calendly_enviado` o `limite_alcanzado`, o si no hay fila en `leads` pero el último mensaje del asistente en el historial ya contenía la URL configurada (`CALENDLY_URL`), la conversación se considera cerrada con Calendly enviado. En ese caso, si el número de mensajes del usuario **posteriores** a ese mensaje del asistente con la URL (incluyendo el mensaje actual) no supera el tope `POST_CALENDLY_FAREWELL_USER_MESSAGES` definido en `config.py` (variable de entorno, valor por defecto `2`), el bot llama al LLM con este system prompt y el historial completo para generar una despedida breve. Si se supera ese tope, el bot responde con el mensaje fijo `MSG_CLOSED_CALENDLY_OR_LIMIT_TEMPLATE` (con `{calendly_url}`) **sin** llamar al LLM.

```
Eres Alejandro, del Estudio Sinergia (diseño de interiores y construcción residencial en Colombia).
